# 📖 Documentation pédagogique — Outil EMI Expert v4.2
...
> - **llama-3.3-70b** (70 milliards de paramètres) : plus lent mais meilleur sur les
...
> `len()` compte les éléments. Si ≥ 12 mots → modèle 70b, sinon → GPT OSS 20B.
...
> Mis à jour en v4.2 pour refléter le nouveau nom du modèle rapide.
...
récapitulatif de l'architecture technique (mis à jour en v4.2), et la section
"Pour aller plus loin" vers le Kit Esprit Critique.

### Tableau architecture (mis à jour v4.2)

```python
| **Groq** (GPT OSS 20B)    | Analyse rapide (requêtes courtes)   | ✅ |
| **Groq** (llama-3.3-70b)  | Analyse complète (requêtes longues) | ✅ |
| **Tavily**                 | Recherche web temps réel            | ✅ |
```
> Remplace l'ancienne ligne `llama-3.1-8b-instant` par `GPT OSS 20B`.
...
| **Groq** (GPT OSS 20B) | Modèle IA rapide | 14 400 req/jour | `GROQ_API_KEY` |
| **Groq** (llama-3.3-70b) | Modèle IA complet | 14 400 req/jour | `GROQ_API_KEY` |
...
*Documentation générée avec Claude (Anthropic) — Mai 2026, mise à jour v4.2 — Juin 2026*
